...
def advance_pointer(increment=False):
  available = [x for x in images_mapping if x not in refs or not refs[x][DOOMSDAY_COUNT] > 0]
  curr_idx = None
  if len(available) > 0:
    has_doomsday_at_one = [x for x in images_mapping if x in refs and refs[x][DOOMSDAY_COUNT] == 1]
    ...
    has_big_doomsday = [x for x in images_mapping if refs[x][DOOMSDAY_COUNT] > 0]
